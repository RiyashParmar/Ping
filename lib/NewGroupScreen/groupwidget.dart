...
class _GroupState extends State<Group> {
  ...
  Future<http.Response> _createGroup(user, members) async {
    File image = File(dp);
                ...
                child: Column(
                  children: [
                    ...
                    Padding(
                      ...
                      child: Row(
                        children: const [
                          Text('Pick a group name, image, description'),
                        ],
                      ),
                    ),
                  ],
          ...
          ? FloatingActionButton(
              ...
              onPressed: () async {
                if (_formkey.currentState!.validate()) {
                  var res = await _createGroup(me, members(widget.user));
                  var id = json.decode(res.body)['id'];
                  if (res.statusCode == 200) {
                    ChatRoom grp = ChatRoom(
                      id_: id,
                      name: _controller1.text.trim(),
                      type: 'group',
                      dp: dp,
